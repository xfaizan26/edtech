...
const Catalog = () => {
// catalog stores the name of current category 
...
// whenever catalog changes category_id changes and therefore pagedetails changes 
  const fetchSublinks=  async ()=>{
    try {
        const result = await apiConnector("GET",categories.CATEGORIES_API);
        const category_id= result.data.data.filter((item)=>item.name=== Catalog.catalog)[0]._id;
        // finding the current category id 
