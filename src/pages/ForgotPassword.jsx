...
//initially we are at reset pass page in which we will  ask user to enter email and AS SOon as user submits form we set emailsent to true and therefore we will go  to resend email page
const ResetPassword = () => {
    ...
    //doubt why are we using dispatch above it does not belong to any slice
    
    //loading is a state that is displayed when the frontend is fetching api from backend therefore the time durinmg it we display loading
const {loading}= useSelector((state)=>state.auth)
    const [emailSent, setemailSent] = useState(false)
    const [email, setemail] = useState("");
    // we use email to display email in the form initially empty
    //we will use conditional rendering everywhere
  return (
    <div className='grid min-h-[calc(100vh-3.5rem)] place-items-center'>
            {
                ...
                (<div className='max-w-[500px] p-4 lg:p-8'>
                    <h1 className='text-[1.875rem] font-semibold leading-[2.375rem] text-richblack-5'>
                        {   //if email is sent we display reset your pass else check email
