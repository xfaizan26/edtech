...
const RenderSteps = () => {
    // extract the current step from course slice 
  ...
  return (
    ...
    <div className=' flex flex-col w-[calc(100vw-20%)] md:w-fit items-start'>
        <div className=' ml-10 relative mb-2 flex w-full justify-center'>
            {/* if the current item and step matches then yellow else normal  */}
            {steps.map( (item) => (
                <div key={item.id} className=' flex w-full justify-between'>
                    <div className='flex flex-col items-center'>
                        ...
                        {/* if the current step is greater than item id then checked icon appears else step number appears  */}
                        </div>
                    </div>
                    {/* if item id < 3 then they will be connected with dots if completed then yellow else white  */}
                    {item.id <3 && (
        ...
        <div className='relative mb-16 flex w-full select-none justify-between'>
            {steps.map((item) => (
                <>
                    ...
                    {/* if step==1 then coursw info form is rendwered if 2 next and so on  */}
